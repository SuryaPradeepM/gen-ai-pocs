...
function LogoGenerator({ classes }) {
  ...
  const prepareFormData = () => {
    return {
      ...formData,
      preferred_symbols: formData.preferred_symbols.filter((item) =>
        item.trim(),
      ),
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_LOGO_GENERATOR_ENDPOINT}/api/v1/generate`,
        {
          method: "POST",
          headers: {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_LOGO_GENERATOR_ENDPOINT}${logoData.logo_url}`,
      );
        ...
        <Grid item className={classes.mainContent}>
          ...
          <Box className={classes.formContainer}>
            <Box className={classes.inputSection}>
              ...
              {/* Chip Input Fields */}
              {["preferred_symbols", "avoid_elements", "brand_values"].map(
                (field) => (
