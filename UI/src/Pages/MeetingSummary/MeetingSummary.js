...
function MeetingSummary(props) {
    ...
    const [classifiedInputValue, setClassifiedInputValue] = useState(`Generate meeting minutes using a provided transcript from a meeting. Please ensure all relevant and critical details of the meeting are kept intact.`);
    ...
    const handleSubmitDocumentClassification = async () => {
        ...
        try {
            ...
            if (getDocClassificationInfo && getDocClassificationInfo.status === 200) {
                ...
                const errorMessage = (getDocClassificationInfo && getDocClassificationInfo.status === 1101) 
                    ? 'Response Timeout' 
                    : (getDocClassificationInfo?.data?.detail || 'An error occurred while processing your request');
            ...
            setErrorMsgText('Failed to connect to the server. Please check if the server is running.');
            setSnackBar({ ...snackBar, open: true });
            setvalidatepdfBtn(true);
        } finally {
    ...
    const downloadPDF = () => {
        ...
        const headingToBreakBefore = ['Agenda:', 'Main Points of Discussion:', 'Decisions Made:'];
        ...
        html2pdf()
            .from(contentElement).set(opt).save();
    };
        ...
        <Grid item className={classes.mainSection}>
            <Scrollbars>
                ...
                <Grid item className={classes.mainContent}>
                    <Typography className={classes.mainTitle} variant='h1' component={'h1'}><Home onClick={() => { navigate('/') }} className={classes.homeIcon} /> Meeting Minutes</Typography>
                        ...
                        <Grid item xs={4}>
                            <Grid item className={classes.attachedBlock}>
                                ...
                                <Grid item className={classes.attachedTop}>
                                    <Grid item className={classes.labelBox}>
                                        <Typography variant='body1'>Attach Meeting Transcript VTT document<sup>*</sup></Typography>
                                    </Grid>
                                ...
                                <Grid item className={classes.fieldParentTextfiled}>
                                    <Grid item className={classes.labelBox}>
                                        <Typography variant='body1'>Meeting Minutes Prompt<sup>*</sup></Typography>
                                    </Grid>
                                    <TextField
                                        id="outlined-multiline-static"
                                        multiline
                            ...
                            <Grid item className={classes.formBlock}>
                                <LoadingButton 
                                    ...
                                    variant="outlined" 
                                    disabled={!transcriptName || documentClassificationLoader || !classifiedInputValue} 
