...
const VoiceConversion = ({ sessionId, fileUploaded, goToNextSlide, totalslides }) => {
  ...
  const preloadNextSlideAudio = async (nextSlide) => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_VIRTUAL_PRESENTER_ENDPOINT}/explain_slide/${sessionId}/${nextSlide}`);
        ...
        synthesizer.speakTextAsync(
          ...
          (result) => {
            if (result.reason === speechSdk.ResultReason.SynthesizingAudioCompleted) {
              ...
              setAudioQueue((prevQueue) => [...prevQueue, { slide: nextSlide, audioUrl, text }]);
  ...
  const handlePresentation = async () => {
    ...
    if (audioQueue?.length > 0 && audioQueue[currentSlide]?.slide === currentSlide && currentSlide >= 1 || audioQueue.length > currentSlide +1) {
      ...
      try {
        setUploading(true);
        const response = await axios.get(`${process.env.REACT_APP_VIRTUAL_PRESENTER_ENDPOINT}/explain_slide/${sessionId}/${currentSlide}`);
        if (response.status === 200) {
  ...
  return (
    <Box sx={{ height: '78vh', paddingBottom: '40px', width: '25%', marginTop: "32px" }}>
      <Paper elevation={3} sx={{ padding: 2 }}>
        {!presentationStarted &&
          <Button
            ...
            disabled={presentationStarted && disableNext || !sessionId || currentSlide > totalslides}
          >
        ...
        {presentationStarted && (
          <Box>
            ...
            <Button
              ...
              sx={{ marginLeft: 2,marginRight: 2, textTransform: "capitalize" }}
              disabled={isPlaying || isListening || !sessionId || uploading || presentationStarted && disableNext}
            >
              {isListening ? "Listening..." : "Ask a Question"}
            </Button>
            <IconButton
              onClick={handlePresentation}
              disabled={disableNext || !sessionId || currentSlide > totalslides}
      ...
      <audio
        ...
        controls
        autoPlay
        onEnded={() => { setDisableNext(false); setIsPlaying(false) }}
